...
interface AuthApi {
    ...
    @DELETE("api/v12/reservas/{id}")
    suspend fun cancelarReserva(@Path("id") id: Long): Response<Unit>

    @PUT("api/v12/pagos/{id}")
    suspend fun completarPago(
        @Path("id") id: Long,
        @Body request: PagoConfirmarRequest
    ): Response<Unit>

    @GET("api/v12/pagos/reserva/{idReserva}")
    suspend fun obtenerPagoPorReserva(@Path("idReserva") idReserva: Long): Response<PagoResponse>
}
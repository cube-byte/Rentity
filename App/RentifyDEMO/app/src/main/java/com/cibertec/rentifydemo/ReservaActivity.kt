...
class ReservaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btnConfirmarReserva).setOnClickListener {
            ...
            if (fechaFin!!.timeInMillis == fechaInicio!!.timeInMillis || !fechaFin!!.after(fechaInicio!!)) {
                Toast.makeText(this, "La fecha de fin debe ser diferente y posterior a la de inicio", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            lifecycleScope.launch {
                try {
                    ...
                    if (response.isSuccessful) {
                        val reservaId = response.body()?.id ?: 0L
                        val precio    = response.body()?.precio_total ?: 0.0

                        // Buscar el pago asociado
                        val pagoResponse = RetrofitInstance.api.obtenerPagoPorReserva(reservaId)
                        val pagoId = pagoResponse.body()?.id ?: 0L

                        Toast.makeText(this@ReservaActivity, "¡Reserva creada! Procede al pago", Toast.LENGTH_SHORT).show()
                        val pagoIntent = Intent(this@ReservaActivity, PagoActivity::class.java)
                        pagoIntent.putExtra("pago_id",        pagoId)
                        pagoIntent.putExtra("reserva_id",     reservaId)
                        pagoIntent.putExtra("reserva_precio", precio)
                        pagoIntent.putExtra("auto_nombre",    "$marca $modelo")
                        pagoIntent.putExtra("fecha_inicio",   inicio)
                        pagoIntent.putExtra("fecha_fin",      fin)
                        startActivity(pagoIntent)
                        finish()
                    } else {
                        val errorMsg = response.errorBody()?.string() ?: "Error al reservar"
                        if (errorMsg.contains("no está disponible")) {
                            Toast.makeText(this@ReservaActivity, "Este auto ya está reservado", Toast.LENGTH_LONG).show()
                        } else {
